...
This module covers the **Design Artifact Schema** — the formal handoff contract between the Exploration Environment and the Freeze Pipeline. Per §3.3, the Exploration Environment produces a **Design Artifact** (YAML) that captures both the intended specification and the AI's uncertainty about each element. It is the structure the Freeze Pipeline (§4) scans for `fuzzy_nodes` and `llm_reasoning` Jobs, and the structure the Freeze Pipeline rejects if any fuzzy node remains unresolved.
...
**In-scope:**
- §3.3 Design Artifact Schema (Handoff Contract) — the YAML structure produced by the Exploration Environment and consumed by the Freeze Pipeline.
...
- The Exploration Environment NL→Spec authoring UX that produces the artifact → not owned by `workflow-engine`.

**Upstream/downstream neighbors:**
- *Producer*: Exploration Environment (user + copilot) emits the artifact in `draft` status.
- *Consumer*: Freeze Pipeline — scans `fuzzy_nodes` + `llm_reasoning` Jobs, requires all `user_confirmed == true`, advances status to `frozen`.

## Interfaces

### §3.3 Design Artifact — YAML contract

The Exploration Environment produces a **Design Artifact** (YAML) that serves as the formal contract between Exploration Environment and Freeze Pipeline. It captures both the intended specification and the AI's uncertainty about each element.
...
- **`confidence_summary.overall`**: drives Freeze Pipeline review-depth gating (`< 0.8` → full peer review).
- **Freeze Pipeline** ([`freeze-pipeline.md`](freeze-pipeline.md) §4): the consumer that enforces "all `fuzzy_nodes.user_confirmed == true`" before proceeding (step 4 of §21.1).
...
- **`artifact`** — top-level envelope.
  ...
  - `status` — lifecycle: `draft` | `user_reviewed` | `frozen`. Advanced to `frozen` only by the Freeze Pipeline.
...
- **`fuzzy_nodes[]`** — one per AI-unresolvable node.
  ...
  - `user_confirmed` — boolean; **must be `true` for Freeze Pipeline to proceed**.
...
| --- | --- | --- |
| Artifact submitted with any `fuzzy_nodes[].user_confirmed == false` | Freeze Pipeline cannot proceed | Freeze Pipeline rejects the artifact (step 4 of §21.1: requires `all fuzzy_nodes.user_confirmed == true`). User must resolve each via the Spec Refinement Assistant (§4.1 Step 4 — Mandatory). |
...
| `confirmed_fields` conflicts with downstream `fuzzy_nodes` | Ambiguous authority | `confirmed_fields` are authoritative — Freeze Pipeline skips AI re-evaluation for them. Conflicts surface via the §4.3.3 Conflict Resolution Rule. |
| `status` advanced prematurely (e.g., `frozen` without sign-off) | Invalid lifecycle transition | Status transitions are gated by the Freeze Pipeline; only `freeze()` advances to `frozen`, and only after all gates pass. |
...
- **Fuzzy-node resolution**: `100%` of `fuzzy_nodes` must have `user_confirmed == true` before Freeze Pipeline proceeds (hard gate).
...
- **`confirmed_fields` authority**: Freeze Pipeline treats `confirmed_fields` as authoritative, skipping AI re-evaluation (zero re-check cost).
...
```
Exploration Environment (user + copilot)
    │
    ▼
┌─────────────────────────────────────────────┐
...
- **Glossary** ([../../glossary.md](../../glossary.md)): Design Artifact, Handoff Contract, Fuzzy Node, `confirmed_fields`, `confidence_summary`, Freeze Pipeline, Spec Refinement Assistant.
