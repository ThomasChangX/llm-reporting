...
- *Read path*: AI Copilot (Exploration Environment) NL→Intent → Query Generator → ValidatedSQL → Query Rewriter (RLS/CLS/masking) → Data Connector Adapter → ResultSet → Compute Engine post-processing → FinalResult.
...
| **AI Copilot (Exploration Environment)** | Conversation Interface's Intent Parser passes NL→Intent to Query Generator; Query Generator returns ValidatedSQL for Preview execution   | NL Intent → SQL          |
...
- **Query Generator**: KB Retrieval (Business Glossary formulas, dimension lookups); Data Catalog (table/column location); Mapping Registry (cross-system mapping); Relationship Graph (JOIN paths); Exploration Environment fallback for `AMBIGUOUS_JOIN_PATH`.
...
| `AMBIGUOUS_JOIN_PATH` (Query Generator Step 2) | Cannot auto-select JOIN path | Fallback to Exploration Environment for user selection. |
...
│   AMBIGUOUS_JOIN_PATH, fallback to Exploration Environment   │
│   for user selection                              │
│                                              │
