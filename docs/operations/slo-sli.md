...
| **SLI Measurement** | **(a)** Prometheus counter `workflow_execution_total{status="..."}` where status is one of `success`, `failed`, `timeout`. **(b)** Histogram `workflow_execution_duration_seconds` with buckets [60, 300, 600, 1800, 3600, 7200, 14400]. Segmented by `workflow_type`. |
